...
#### モック由来の確定情報（[`mockups/`](./mockups/) 参照）

| 項目 | 確定値 |
|---|---|
| プロダクト正式名 | **SABOROU**（英字大文字） |
| タグライン | 「- サボりの最適解 -」 |
| ロゴアイコン | 🛏️ |
| 主要カラー | オレンジ（CTA・ロゴ）/ オフホワイト（背景）/ 淡黄（強調ボックス）/ 白（カード）/ 赤（破壊的アクション） |
| レイアウト前提 | デスクトップ向け2ペイン構成（モバイルは縦積みに切替必要） |
| MVP 画面数 | 2画面（タスク一覧 / タスク詳細＋チャット） |
...
| 観点 | Bedrock AgentCore | Strands Agent SDK + Hono | 自前実装（Hono + Bedrock SDK） |
|---|---|---|---|
| マルチエージェント協調 | ◎ ネイティブサポート | △ 協調制御を追加実装 | △ 協調制御を全て自前実装 |
| セッション管理 | ◎ マネージド | ○ 外部ストア設計が必要 | △ 完全自前設計 |
| AWS統合 | ◎ ネイティブ統合 | ○ SDK経由で実装可能 | ○ SDK経由で実装可能 |
| 運用負荷 | ◎ 低い（マネージド） | △ 中程度 | △ 高い |
| 開発速度（ハッカソン） | ◎ 速い | ○ 実装量は中程度 | △ 実装量が多い |
| 柔軟性 | ○ | ○ | ◎ |

**決定理由**:
- Dual-Agent 構成（タスク抽出エージェント→サボり提案エージェント）の協調を最短で実装できる
- セッション・実行コンテキスト管理をマネージドに寄せ、ハッカソン期間での実装リスクを下げられる
- AWSサービスとの統合性が高く、Lambda / EventBridge / DynamoDB との設計整合が取りやすい
- 自前実装で必要な基盤コードを削減し、UXとデモ品質に工数を集中できる
