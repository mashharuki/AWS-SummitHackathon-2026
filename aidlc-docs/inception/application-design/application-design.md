...
SABOROU は「タスク抽出」と「サボり提案」の2エージェントが協調するサーバーレスアーキテクチャを採用する。

フロントエンドは S3 + CloudFront で配信され、バックエンドは Hono on Lambda + API Gateway HTTP API で構成される。2つの AI エージェントはそれぞれ独立した Lambda として動作し、Bedrock AgentCore を通じて Claude Sonnet を呼び出す。データ層は DynamoDB On-Demand モード（PAY_PER_REQUEST）を使用する。
...
[Hono on Lambda: apps/api]
    ...
    +── [DynamoDB] ← 永続化
    |
    +── [Lambda: TaskExtractorAgent]  ← エージェント①
    |       └── [Bedrock AgentCore / Claude Sonnet]
    |
    +── [Lambda: SaboriProposerAgent] ← エージェント②
    |       └── [Bedrock AgentCore / Claude Sonnet]
    |
...
| 原則 | 適用内容 |
|------|---------|
| **エージェント抽象化** | `ITaskExtractorAgent` / `ISaboriProposerAgent` インタフェースで実装を差し替え可能にする。AgentCore 障害時は Bedrock InvokeModel に自動フォールバック |
...
外部サービス（Slack / Gmail / Google Calendar）
  → Webhook Lambda → EventBridge → TaskExtractorAgent（エージェント①）
                                    → Bedrock AgentCore
                                    → DynamoDB Tasks テーブル
...
| バックエンド | BE-06 | WebhookHandler | Slack Webhook 受信 |
| エージェント | AG-01 | TaskExtractorAgent | 外部メッセージ → タスク候補変換 |
| エージェント | AG-02 | SaboriProposerAgent | 文脈統合 → サボり提案生成 |
| エージェント | AG-03 | PersonaRenderer | おっとりサボロー口調変換 |
...
Units Generation ステージで確定するが、現時点での想定は以下の5 Unit。

| Unit | モジュール | 主な作業内容 |
|------|----------|------------|
| Unit-01: shared | packages/shared | 型定義・共通ユーティリティ・エラークラス |
| Unit-02: infra | infra/ | CDK 全スタック（Cognito / Data / Api / Agent / Frontend / Webhook）|
| Unit-03: agent | packages/agent | TaskExtractorAgent / SaboriProposerAgent / PersonaRenderer / ContextCollector |
| Unit-04: api | apps/api | Hono ハンドラ全6コンポーネント・サービス層 |
| Unit-05: web | apps/web | React 全画面（TaskList / TaskDetail / Login / Settings / AppShell）|

**依存順序**: Unit-01 → Unit-02 → Unit-03 → Unit-04 → Unit-05
