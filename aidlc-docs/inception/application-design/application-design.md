...
| **モノレポ型依存管理** | pkgs/shared が型定義の唯一の真実の源。循環依存禁止 |
...
graph TD
    ...
    subgraph Frontend["pkgs/frontend（CloudFront + S3）"]
        AppShell["AppShell"]
    ...
    subgraph Backend["pkgs/backend（Hono on Lambda）"]
