...
| 実装方針 | モノレポ（pkgs/ 配下に全パッケージ統合）pnpm workspaces@10.33.0 |
...
| U-04 | api | Hono on Lambda による REST API + Webhook ハンドラの実装 | U-01, U-02, U-03a, U-03b | L | 高 |
...
レイヤー 5（U-01 + U-02 + U-03a + U-03b 依存）:
  U-04: api ← U-01, U-02, U-03a, U-03b（U-03c は v1.1.0 スコープのため除外）
...
**Unit ID**: U-01
**Unit 名**: shared
**ディレクトリ**: `pkgs/shared/`（Construction フェーズで新規作成）
**規模**: S（Small）
**推定工数**: 2〜3時間
...
| INF-04: AgentStack | TaskExtractorAgent Lambda + SaboriProposerAgent Lambda + BackgroundRefreshHandler Lambda + IAM ロール設定 + Bedrock converse API アクセス権限 |
...
Cognito / DynamoDB / Lambda / API Gateway HTTP API / S3 / CloudFront / EventBridge / Secrets Manager / Bedrock（converse API）/ CloudWatch / IAM
...
**ディレクトリ**: `pkgs/agent/`（Construction フェーズで新規作成）
**規模**: M（Medium）
**推定工数**: 4〜6時間
...
- U-02 infra: Lambda 実行ロール ARN（環境変数）/ DynamoDB テーブル名（環境変数）/ Secrets Manager ARN（環境変数）
...
| コンポーネント（AG-ID） | 内容 |
|----------------------|------|
| AG-05: TaskOrganizerAgent | タスク候補リスト → 依存関係グラフ・推奨実行順序・サボり余地スコア（0〜100）の計算。Bedrock converse API + Tool Use 使用 |
...
- U-02 infra: Lambda 実行ロール ARN（環境変数）/ DynamoDB テーブル名（環境変数）
- U-03a task-extractor: TaskExtractorAgent Lambda の出力（TaskCandidate[]）/ IBedrockClient 実装（再利用）
...
Lambda / Bedrock converse API + Tool Use / DynamoDB / EventBridge（受信トリガー・送信）
...
- [ ] TypeScript コンパイルエラーなし
- [ ] converse API + Tool Use によるタスク整理ロジックが正常動作
...
- U-02 infra: Lambda 実行ロール ARN（環境変数）/ DynamoDB テーブル名（環境変数）/ Secrets Manager ARN（環境変数）
- U-03a task-extractor: IBedrockClient 実装（`src/bedrock/client.ts` を再利用）
...
**Unit ID**: U-04
**Unit 名**: api
**ディレクトリ**: `pkgs/backend/`（実装済み — Hono ベースのみ）
**規模**: L（Large）
**推定工数**: 8〜12時間
...
> U-03c（task-organizer）は v1.1.0 スコープのため、U-04 の直接依存から除外する。U-03c が実装された場合は U-03b 経由で間接的に活用される。
...
| コード品質 | Biome 1.9.4 | フォーマッター + リンター（Prettier/ESLint 代替）|
...
**Unit ID**: U-05
**Unit 名**: web
**ディレクトリ**: `pkgs/frontend/`（実装済み — React + Vite ベースのみ）
**規模**: M（Medium）
...
| Unit | リスク | 深刻度 | 緩和策 |
|------|--------|--------|--------|
| U-02: infra | Bedrock converse API の ap-northeast-1 レイテンシ | 低 | Claude Sonnet 3.5 は ap-northeast-1 で GA 済み（2026年5月現在）。モデルアクセス有効化確認済み |
...
├── pkgs/
│   ├── shared/           ← U-01（Construction フェーズで新規作成）
│   │   ├── src/
│   │   │   ├── types/
│   │   │   ├── schemas/
│   │   │   ├── errors/
...
│   │   └── tsconfig.json
│   │
│   ├── agent/            ← U-03a / U-03c / U-03b（Construction フェーズで新規作成）
│   │   ├── src/
...
│   │   └── package.json
│   │
│   ├── backend/          ← U-04（実装済み — Hono ベースのみ）
│   │   ├── src/
│   │   │   ├── index.ts       ← ローカル開発サーバー（@hono/node-server）
│   │   │   ├── handler.ts     ← Lambda ハンドラ（hono/aws-lambda）
│   │   │   └── config/
│   │   │       └── openapi.ts ← OpenAPI 仕様定義
│   │   ├── package.json
│   │   └── tsconfig.json
│   │   ← Construction で routes/ / repositories/ / middleware/ を追加
│   │
│   ├── frontend/         ← U-05（実装済み — React + Vite ベースのみ）
│   │   ├── src/
│   │   │   ├── App.tsx
│   │   │   └── main.tsx
│   │   ├── package.json
│   │   └── vite.config.ts
│   │   ← Construction で pages/ / components/ / providers/ / lib/ を追加
│   │
│   └── cdk/              ← U-02（実装済み — スケルトンのみ）
│       ├── bin/
│       │   └── cdk.ts         ← CDK アプリ エントリポイント
│       ├── lib/
│       │   └── cdk-stack.ts   ← スタック定義（現在空）
│       ├── package.json
│       └── cdk.json           ← CDK コンテキスト・フィーチャー設定
│       ← Construction で lib/stacks/ 配下に 6 スタックを追加
│
├── biome.json            ← コード品質統一設定（Biome 1.9.4）
├── pnpm-workspace.yaml   ← ワークスペースルート（pnpm workspaces@10.33.0）
└── package.json          ← ルート設定（Biome / 共通スクリプト）
```
