...
**作成日**: 2026-05-09
**バージョン**: 1.1.0
**更新日**: 2026-05-10（U-03c: task-organizer 追加 / Unit 総数 6→7）
**ステータス**: 承認待ち
...
| U-03c | task-organizer | タスク依存関係・手順最適化・サボり余地スコア計算エージェントの実装 ★新規 | U-01, U-02, U-03a | M | 高 |
| U-03b | sabori-proposer | サボり提案・口調変換・再評価エージェントの実装（人格A/B対応）| U-01, U-02, U-03a, U-03c | M | 高 |
...
### U-03c: task-organizer（タスク整理エージェント）★新規追加（v1.1.0）
...
TaskExtractorAgent（U-03a）が収集した生タスクリストを受け取り、タスク間の依存関係・実行手順・優先順位を整理・構造化する中間エージェント。「どの順番でタスクを処理すれば最も長くサボれるか」を計算し、SaboriProposerAgent（U-03b）に精度向上のためのコンテキストとして引き渡す。
...
サボり提案フローの実装。TaskOrganizerAgent（U-03c）からの整理済みタスクプランを受け取り、ContextCollector（AG-04）で外部API文脈を収集し、SaboriProposerAgent（AG-02）が verdict と reasoning を生成、PersonaRenderer（AG-03）が人格A/B の口調へ変換する。BackgroundRefresh による再評価も担当する。

#### 含まれるコンポーネント

| コンポーネント（AG-ID） | 内容 |
|----------------------|------|
| AG-02: SaboriProposerAgent | 整理済みタスクプラン（U-03c出力）+ コンテキスト統合 → `can_saboru` / `caution` / `danger` 判定 + reasoning 生成。ストリーミング対応（Bedrock Streaming API）|
