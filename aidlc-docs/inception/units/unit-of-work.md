...
| Unit 総数 | 6（U-01, U-02, U-03a, U-03b, U-04, U-05）|
| 実装方針 | モノレポ（packages/ + apps/ + infra/）|
| 依存順序 | shared → infra → task-extractor → sabori-proposer → api → web |
...
| U-03a | task-extractor | Bedrock AgentCore を用いたタスク抽出エージェントの実装 | U-01, U-02 | M | 高 |
| U-03b | sabori-proposer | サボり提案・口調変換・再評価エージェントの実装 | U-01, U-02, U-03a | M | 高 |
...
レイヤー 3（U-01 + U-02 + U-03a 依存）:
  U-03b: sabori-proposer ← U-01, U-02, U-03a

レイヤー 4（U-01 + U-02 + U-03a + U-03b 依存）:
  U-04: api ← U-01, U-02, U-03a, U-03b

レイヤー 5（U-01 + U-04 依存）:
  U-05: web ← U-01, U-04（型契約のみ使用）

実装順序: U-01 → U-02 → U-03a → U-03b → U-04 → U-05
並行可能: U-03a 完了後、U-03b と U-04 の設計を並行開始可
```
...
Bedrock AgentCore SDK を用いたタスク抽出フローの実装。Slack / Gmail / Calendar 由来イベントを構造化タスク候補へ変換する AG-01 を中心に、Bedrock クライアントラッパーと TaskExtractor Lambda ハンドラを提供する。
...
| ステージ | 担当内容 |
|---------|---------|
| Functional Design | タスク抽出プロンプト設計 / タスク候補スキーマ設計 / フォールバック条件定義 |
| NFR Requirements | 抽出遅延10秒以内 / トークン制限ガード / 月次コスト上限 |
| NFR Design | Bedrock フォールバック制御フロー / 失敗時リトライ戦略 |
| Infrastructure Design | TaskExtractor Lambda メモリ・タイムアウト設定 |
| Code Generation | `src/agents/task-extractor.ts` / `src/bedrock/client.ts` / TaskExtractor Lambda ハンドラ / Vitest 単体テスト |
...
サボり提案フローの実装。ContextCollector（AG-04）で外部API文脈を収集し、SaboriProposerAgent（AG-02）が verdict と reasoning を生成、PersonaRenderer（AG-03）が `おっとりサボロー` 口調へ変換する。BackgroundRefresh による再評価も担当する。
...
| Lambda ハンドラ | SaboriProposerAgent Lambda エントリポイント / BackgroundRefreshHandler Lambda エントリポイント |
...
- U-01 shared: `Task` / `Proposal` / `Verdict` 型 / `guardTokenLimit()` / `pseudonymize()` / `BedrockTimeoutError`
- U-02 infra: Bedrock AgentCore エンドポイント / Lambda 実行ロール ARN（環境変数）/ DynamoDB テーブル名（環境変数）/ Secrets Manager ARN（環境変数）
...
Lambda / Bedrock AgentCore / Bedrock InvokeModel（フォールバック）/ DynamoDB / Secrets Manager / EventBridge（Scheduler）

#### 想定実装ステップ（Construction 各ステージ）

| ステージ | 担当内容 |
|---------|---------|
| Functional Design | サボり判定ロジック3状態の詳細設計 / next_check_at 計算ルール / プロンプトテンプレート設計 |
| NFR Requirements | Bedrock レイテンシ要件 / 生データ削除タイミング / 月次コスト上限 |
| NFR Design | `Promise.allSettled` 並列設計 / Bedrock Streaming 実装方針 |
| Infrastructure Design | SaboriProposer / BackgroundRefresh の Lambda メモリ・タイムアウト設定 |
...
| U-03b: sabori-proposer | 設計書完成（Inception）| SaboriProposerAgent + PersonaRenderer MVP 動作 | BackgroundRefresh 完成 |
...
| U-03b | `[Construction] U-03b: sabori-proposer — サボり提案エージェント実装（AG-02/03/04 + BackgroundRefresh）` |
