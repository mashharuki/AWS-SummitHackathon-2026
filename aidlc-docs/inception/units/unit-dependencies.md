...
**作成日**: 2026-05-09
**バージョン**: 1.1.0
**更新日**: 2026-05-10（U-03c: task-organizer を追加）
...
| 依存元 \ 依存先 | U-01 shared | U-02 infra | U-03a task-extractor | U-03c task-organizer | U-03b sabori-proposer | U-04 api | U-05 web |
|--------------|:-----------:|:----------:|:---------------------:|:---------------------:|:---------------------:|:--------:|:--------:|
| U-01 shared  | -           | -          | -                     | -                     | -                     | -        | -        |
| U-02 infra   | ○（型参照）  | -          | -                     | -                     | -                     | -        | -        |
| U-03a task-extractor | ○（型・ユーティリティ）| ○（環境変数・ARN）| - | - | - | - | - |
| U-03c task-organizer | ○（型・ユーティリティ）| ○（環境変数・ARN）| ○（TaskExtractor 出力を入力とする）| - | - | - | - |
| U-03b sabori-proposer | ○（型・ユーティリティ）| ○（環境変数・ARN）| ○（Bedrock wrapper再利用）| ○（OrganizedTaskPlan 取得 v1.1.0）| - | - | - |
| U-04 api     | ○（型・インタフェース）| ○（環境変数・ARN）| ○（TaskExtractor invoke）| ○（TaskOrganizer invoke v1.1.0）| ○（SaboriProposer invoke） | - | - |
| U-05 web     | ○（型定義）  | -          | -                     | -                     | -                     | ○（HTTP契約）| -    |
...
### U-03c → U-01（型・ユーティリティ）[v1.1.0]

| 依存内容 | 用途 |
|---------|------|
| `Task` / `TaskCandidate` / `OrganizedTaskPlan` 型 | エージェントの入出力型 |
| `guardTokenLimit()` | Bedrock プロンプトトリム |
| `pseudonymize()` | 依頼者名の仮名化 |

**結合度**: 高（型が変わると organizer の実装変更が必要）
**スコープ**: v1.1.0（M3 決勝）

---

### U-03c → U-02（環境変数・ARN）[v1.1.0]
...
### U-03c → U-03a（TaskExtractor 出力を入力とする）[v1.1.0]

| 依存内容 | 用途 |
|---------|------|
| DynamoDB TaskCandidates テーブルのレコード | 依存関係分析・サボり余地スコア計算の入力 |

**呼び出し方法**: EventBridge 経由でトリガー（U-03a 完了後に `task.extracted.completed` イベントを受信）
**結合度**: 低（EventBridge 経由の疎結合。インタフェースは DynamoDB スキーマ契約のみ）
**スコープ**: v1.1.0（M3 決勝）

---

### U-03b → U-03c（OrganizedTaskPlan 取得）[v1.1.0]

| 依存内容 | 用途 |
|---------|------|
| DynamoDB TaskOrganization テーブルのレコード | サボり提案時の依存関係情報・サボり余地スコアの参照 |

**結合度**: 低（DynamoDB 直接参照。v1.0.0 では null として無視する設計）
**スコープ**: v1.1.0（M3 決勝）。v1.0.0 では U-03b が TaskOrganization テーブルを参照しない。
...
```
U-01 → U-02 → U-03a → U-03c（v1.1.0）→ U-03b → U-04 → U-05
```

※ v1.0.0（M2 MVP）では U-03c をスキップし U-03a → U-03b で直結する
...
**U-03c（v1.1.0）が U-03a の後・U-03b の前の理由**:
- U-03c は U-03a の出力（TaskCandidates）を入力として受け取るため U-03a より後
- U-03b は U-03c の出力（OrganizedTaskPlan）を参照するため U-03c より後
- v1.0.0 では U-03c をスキップし U-03a → U-03b で直結する（OrganizedTaskPlan = null）
...
```
フェーズ C（拡張）:
  - C0: U-03c TaskOrganizerAgent 実装（U-03a 完了後・U-03b と並行）
↓
フェーズ C1（U-03b）: OrganizedTaskPlan 参照ロジックを追加統合
```
...
| U-01 → U-02 → ... → U-01 | なし |
| U-02 → U-03a → ... → U-02 | なし |
| U-03a → U-03c → ... → U-03a | なし |
| U-03a → U-03b → ... → U-03a | なし |
| U-03c → U-03b → ... → U-03c | なし |
| U-03b → U-04 → ... → U-03b | なし |
| U-04 → U-05 → ... → U-04 | なし |
| U-05 → U-01 → ... → U-05 | なし |

すべての依存関係は DAG（有向非循環グラフ）として成立している。U-03c 追加後も循環依存は発生しない。
