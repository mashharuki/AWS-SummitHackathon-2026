...
| **MCP Server** | Claude/Cursor から Omi の記憶・会話に直接アクセス | ★★☆ (5分でスタート) |
...
### パターン C: MCP 経由で Claude に記憶を持たせる

```
Omi MCP Server + Claude Desktop
    → Claude から search_memories("プロジェクトXについて")
    → Omi の会話データを直接参照
    → Claude が記憶を持った状態で会話
```
...
## MCP セットアップ (Claude Desktop / Cursor)
